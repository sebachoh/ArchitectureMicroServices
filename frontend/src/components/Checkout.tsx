...
export function Checkout() {
    ...
    return (
        <div className="min-h-screen bg-[#0c0a09] text-stone-100 flex items-center justify-center p-4">
            <div className="max-w-4xl w-full bg-stone-900 border border-white/5 rounded-3xl overflow-hidden shadow-2xl flex flex-col md:flex-row">

                {/* Left: Summary */}
                <div className="flex-1 p-8 border-b md:border-b-0 md:border-r border-white/5 bg-stone-950/30">
                    <button
                        ...
                        {items.map(item => (
                            <div key={item.id} className="flex gap-4 items-center">
                                ...
                                <div className="flex-1">
                                    ...
                                    <p className="text-stone-500 text-sm">Cant: {item.quantity}</p>
                                </div>
                    ...
                    <div className="space-y-4 mb-8">
                        <label className={`block p-4 rounded-xl border-2 cursor-pointer transition-all ${paymentMethod === 'cash' ? 'border-orange-500 bg-orange-500/10' : 'border-white/5 hover:border-white/20'}`}>
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h3 className="font-bold">Efectivo / Contraentrega</h3>
                                    <p className="text-sm text-stone-500">Paga cuando recibas tu pedido</p>
                                </div>
                            </div>
                        </label>
                        ...
                        <label className={`block p-4 rounded-xl border-2 cursor-pointer transition-all ${paymentMethod === 'transfer' ? 'border-orange-500 bg-orange-500/10' : 'border-white/5 hover:border-white/20'}`}>
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-stone-500">Envía el comprobante por WhatsApp</p>
                                </div>
                            </div>
                        </label>
                    </div>
